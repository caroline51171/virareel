// Import de TYPE uniquement (efface a la compilation) : les libelles ne portent
// plus l'emoji, ils portent le NOM de l'icone rendue par <Icon> cote JSX.
import type { IconName } from '@/components/Icon';

export type Lang = 'fr' | 'en';

export const translations = {
  fr: {
    nav: {
      logo: 'ViraReel AI',
      pricing: 'Tarifs',
      referral: 'Parrainage',
      dashboard: 'Tableau de bord',
    },
    hero: {
      badge: 'Outil de productivité pour agences et créateurs actifs',
      title: 'Des scripts complets',
      titleGradient: 'prêts à publier',
      titleEnd: 'en quelques secondes',
      subtitle: 'Saisissez votre idée et le contexte de votre publication (lieu, visuel, ambiance) : l\'IA génère des hooks, scripts, légendes et hashtags ultra-personnalisés pour vos 4 plateformes.',
      cta: 'Accéder au générateur de scripts',
      ctaSub: '12 essais gratuits — Sans courriel ni carte de crédit',
    },
    generator: {
      title: 'Générateur de scripts',
      subtitle: 'Saisissez votre idée ainsi que le contexte de votre visuel pour générer vos textes.',
      // « idée » retiré volontairement : le mot est réservé aux onglets « Idée 1-4 »
      // (deux sens différents dans le même écran = la confusion qu'on cherche à éviter).
      topicLabel: 'Sujet, visuel & cible de votre publication',
      // UN exemple complet plutôt que trois sujets sans contexte : il doit démontrer les
      // 4 points de la fenêtre d'aide, « qui parle » compris.
      topicPlaceholder: 'Ex. : 3 erreurs de marketing qui coûtent cher. Je m\'adresse à des propriétaires de PME. Consultant depuis 8 ans. Je parle face caméra dans un bureau moderne, plans serrés sur le produit.',
      // Fenêtres d'aide « ⓘ » : quoi écrire dans le champ du haut, et en quoi les
      // champs « Idée 1-4 » sont différents (c'est la confusion la plus fréquente).
      topicHelp: {
        title: 'Quoi écrire ici',
        intro: 'Votre sujet et le contexte que l\'IA ne peut pas deviner :',
        bullets: [
          { k: 'Le sujet', v: 'ce que vous allez publier' },
          { k: 'À qui', v: 'le public visé' },
          { k: 'Qui parle', v: 'métier, expérience, ce qui rend la personne légitime' },
          { k: 'Le visuel', v: 'le lieu et le style' },
        ],
        exCreator: 'Ex. créateur : « 3 erreurs de marketing qui coûtent cher. Je m\'adresse à des propriétaires de PME. Consultant depuis 8 ans. Je parle face caméra dans un bureau moderne, plans serrés sur le produit. »',
        exAgency: 'Ex. agence — décrivez le client, pas votre agence : « Contenu pour une clinique dentaire familiale. La dentiste-propriétaire présente elle-même. On parle aux parents du quartier. Ton rassurant, tournage sur place. »',
        guard: 'Décrivez ce que vous faites, pas ce que ça garantit.',
        note: 'En mode 4 idées, ce champ garde seulement le contexte : il s\'applique aux 4.',
      },
      ideaHelp: {
        title: 'Quoi écrire ici',
        intro: 'Un seul sujet précis par onglet — pas de contexte, il est déjà en haut.',
        bullets: [
          '4 sujets différents, pas 4 façons de dire la même chose',
          'Une phrase courte suffit',
          "Pas d’inspiration ? Le bouton « Propose-moi 4 angles » les remplit pour vous, gratuitement et sans utiliser d’essai. Vous pouvez ensuite tout modifier.",
        ],
        // Volontairement l'exemple de topicHelp MOINS le sujet : la seule différence visible
        // entre les deux fenêtres doit être le sujet qui s'en va dans les onglets.
        exContext: 'Ex. — dans le champ du haut : « Je m\'adresse à des propriétaires de PME. Consultant depuis 8 ans. Je parle face caméra dans un bureau moderne, plans serrés sur le produit. »',
        exIdeasIntro: 'Puis, dans les onglets :',
        exIdeas: [
          'Idée 1 — Les 3 erreurs de pub Facebook',
          'Idée 2 — Pourquoi votre site ne convertit pas',
          'Idée 3 — Le budget minimum pour démarrer',
          'Idée 4 — Comment écrire une offre irrésistible',
        ],
      },
      toneHelp: {
        title: 'Quel ton choisir',
        intro: 'Le ton décide de la voix du script, pas du sujet :',
        bullets: [
          { k: 'Éducatif', v: 'problème → solution → détails clés. Le meilleur choix pour vendre un produit ou un service.' },
          { k: 'Expert', v: 'affirmation contre-intuitive → preuve → méthode. Pour un public professionnel, appuyé sur un fait ou un mécanisme.' },
          { k: 'Inspirant', v: 'galère → déclic → résultat. Demande votre propre histoire.' },
          { k: 'Humoristique', v: 'prémisse absurde, punchlines qui s\'enchaînent. Pour qu\'on l\'envoie à un ami.' },
          { k: 'Tendance', v: 'un format du moment, énergie actuelle. Vieillit vite.' },
          { k: 'Émotionnel', v: 'tension → résolution. Demande aussi du vécu.' },
        ],
        note: 'Dans le doute, prenez Éducatif.',
      },
      helpOpen: 'Quoi écrire dans ce champ ?',
      helpToneOpen: 'Quel ton choisir ?',
      helpClose: 'J\'ai compris',
      platformLabel: 'Plateformes cibles',
      platformHint: 'Sélectionnez-en une ou plusieurs',
      toneLabel: 'Ton/Style',
      regionLabel: 'Audience cible',
      regionLabelIcon: 'globe' as IconName,
      regions: {
        'qc': 'QC · Québec',
        'fr': 'FR · France',
        'be': 'BE · Belgique / Wallonie',
        'other-fr': 'Français international',
      } as Record<string, string>,
      tones: {
        educational: 'Éducatif',
        expert: 'Expert',
        inspirational: 'Inspirant',
        funny: 'Humoristique',
        trendy: 'Tendance',
        emotional: 'Émotionnel',
      },
      tonesIcons: {
        educational: 'book-open',
        expert: 'briefcase',
        inspirational: 'flame',
        funny: 'laugh',
        trendy: 'sparkles',
        emotional: 'heart',
      } as Record<string, IconName>,
      platforms: {
        instagram: 'Instagram Reels',
        tiktok: 'TikTok',
        facebook: 'Facebook Reels',
        youtube: 'YouTube Shorts',
        all: 'Les 4 plateformes',
      },
      // Logos de marque officiels ; `all` = les 4 plateformes (grille), pas le
      // selecteur de langue.
      platformsIcons: {
        instagram: 'instagram',
        tiktok: 'tiktok',
        facebook: 'facebook',
        youtube: 'youtube',
        all: 'layout-grid',
      } as Record<string, IconName>,
      generateBtn: 'Générer mon script',
      generateBtnIcon: 'zap' as IconName,
      variationsBtn: 'Générer 3 variations de scripts',
      variationsBtnIcon: 'refresh-cw' as IconName,
      ideasBtn: 'Générer 4 idées',
      ideasBtnIcon: 'lightbulb' as IconName,
      generating: 'Génération en cours...',
      remaining: 'essais restants',
      upgradeBtn: 'Passer à Creator',
      upgradeBtnIcon: 'rocket' as IconName,
      results: {
        hook: 'Hook Percutant',
        hookIcon: 'magnet' as IconName,
        hookSub: 'Les 3 premières secondes = tout',
        script: 'Script Étape par Étape',
        scriptIcon: 'file-text' as IconName,
        scriptSub: 'Max 45 secondes de lecture',
        screenText: 'Texte à l\'Écran',
        screenTextIcon: 'pencil' as IconName,
        screenTextSub: 'Une phrase-choc par page',
        visualInspo: 'Inspiration Visuelle',
        visualInspoIcon: 'lightbulb' as IconName,
        visualInspoSub: 'Idées de plans pour filmer (optionnel)',
        caption: 'Légende + Hashtags',
        captionIcon: 'message-circle' as IconName,
        captionSub: 'Prêt à copier-coller',
        duration: 'Durée Suggérée',
        durationIcon: 'timer' as IconName,
        durationSub: 'Optimisé pour TikTok',
        // Ici 🎵 designait le SON, pas TikTok -> Music (et non le logo TikTok).
        trend: 'Tendance Sonore',
        trendIcon: 'music' as IconName,
        trendSub: 'Pour maximiser la portée',
        bestTime: 'Meilleur Moment',
        bestTimeIcon: 'clock' as IconName,
        bestTimeSub: 'Pour publier et maximiser les vues',
        ytTitle: 'Titre Optimisé',
        ytTitleIcon: 'tag' as IconName,
        ytTitleSub: 'Pour le référencement YouTube',
        seoDescription: 'Description SEO',
        seoDescriptionIcon: 'search' as IconName,
        seoDescriptionSub: 'Pour maximiser la découverte',
        keywords: 'Mots-clés / Tags',
        keywordsIcon: 'key' as IconName,
        keywordsSub: 'Tags YouTube pour plus de vues',
        variation: 'Variation',
        copyBtn: 'Copier',
        copyBtnIcon: 'copy' as IconName,
        copied: 'Copié !',
        copiedIcon: 'check' as IconName,
      },
    },
    pricing: {
      title: 'Nos forfaits',
      subtitle: 'Facturation mensuelle ou annuelle',
      monthly: 'Mensuel',
      annual: 'Annuel',
      save: '2 mois offerts',
      perMonth: '/mois',
      perYear: '/an',
      plans: {
        solo: {
          name: 'Solo',
          desc: 'Pour bien démarrer',
          features: [
            '60 générations/mois',
            'Hook + Script + Texte écran + Légende + Hashtags',
            'Inspiration visuelle par plateforme',
            'Tendances sonores TikTok',
            'Meilleur moment de publication',
            'Possibilité de 3 variations par génération',
            'Les 6 tons',
            'Les 4 plateformes',
            'Les 20 dernières générations (sur cet appareil)',
            'Export — choix de 3 formats (TXT / MD / CSV)',
            'Transcréation bilingue en 1 clic (FR ↔ EN, adaptée par région)',
          ],
          cta: 'Commencer Solo',
          badge: '',
        },
        creator: {
          name: 'Creator',
          desc: 'Pour les créateurs ambitieux',
          features: [
            '160 générations/mois',
            'Hook + Script + Texte écran + Légende + Hashtags',
            'Inspiration visuelle par plateforme',
            'Tendances sonores TikTok',
            'Meilleur moment de publication',
            'Possibilité de 3 variations par génération',
            'Les 6 tons',
            'Les 4 plateformes',
            'Les 40 dernières générations, consultables au complet (sur cet appareil)',
            'Export — choix de 3 formats (TXT / MD / CSV)',
            'Transcréation bilingue en 1 clic (FR ↔ EN, adaptée par région)',
            '4 idées × 4 plateformes en 1 clic (16 contenus d\'un coup)',
          ],
          cta: 'Commencer Creator',
          badge: 'Populaire',
          badgeIcon: 'star' as IconName,
        },
        pro: {
          name: 'Agency',
          desc: 'Pour les agences et les équipes',
          features: [
            '1000 générations/mois',
            'Hook + Script + Texte écran + Légende + Hashtags',
            'Inspiration visuelle par plateforme',
            'Tendances sonores TikTok',
            'Meilleur moment de publication',
            'Possibilité de 3 variations par génération',
            'Les 6 tons',
            'Les 4 plateformes',
            'Les 150 dernières générations, consultables au complet (sur cet appareil)',
            'Export — choix de 3 formats (TXT / MD / CSV)',
            'Transcréation bilingue en 1 clic (FR ↔ EN, adaptée par région)',
            '4 idées × 4 plateformes en 1 clic (16 contenus d\'un coup)',
            'Compte partagé pour ton équipe',
            'Support prioritaire',
          ],
          cta: 'Commencer Agency',
        },
      },
      founder: {
        banner: 'OFFRE DE LANCEMENT · Tarif membre fondateur',
        bannerSub: 'Les 50 premiers abonnés gardent leur prix bloqué À VIE.',
        spots: 'places membre fondateur restantes',
        spotsOnly: '50 places seulement',
        barSpots: '50 places',
        // Le vrai nombre restant, pas une formule vague : quelqu'un qui lit « Plus que
        // 1 place » comprend tout de suite pourquoi le prix peut changer à la page
        // suivante. C'est le levier honnête choisi par Caroline — de vraies places,
        // jamais un faux compte à rebours. `{n}` est remplacé dans Pricing.tsx.
        lastSpots: 'Plus que {n} places fondatrices !',
        lastSpotOne: 'Plus que 1 place fondatrice !',
        lastSpotsIcon: 'flame' as IconName,
        // Offre terminée : remplace le bandeau doré, en sobre. La flamme reste
        // réservée à l'offre EN COURS, elle n'aurait plus de sens ici.
        closedIcon: 'star' as IconName,
        closedTitle: 'Les 50 places fondatrices sont comblées.',
        closedThanks: 'Merci aux 50 premiers créateurs qui ont rejoint ViraReel AI dès le début.',
        closedOver: "L'offre fondatrice est maintenant terminée.",
        closedRegular: "Les tarifs réguliers s'appliquent désormais.",
        tag: 'Membre fondateur',
        notice: 'À vie sur le forfait souscrit — non transférable en cas de changement de forfait.',
        lifetime: 'à vie',
        was: 'Prix normal',
        cta: 'Bloquer mon prix à vie',
        launch: 'Offre de lancement',
        bar: 'Prix bloqué à vie',
        seeBelow: 'Voir plus bas',
      },
    },
    referral: {
      title: 'Programme de Parrainage',
      titleIcon: 'gift' as IconName,
      subtitle: 'Partage ViraReel et gagne des mois gratuits !',
      howTitle: 'Comment ça marche ?',
      steps: [
        { icon: 'link' as IconName, title: 'Copie ton lien unique', desc: 'Chaque compte a un lien de parrainage personnel.' },
        { icon: 'share' as IconName, title: 'Partage-le', desc: 'Envoie ton lien à tes amis, abonnés, ou sur les réseaux.' },
        { icon: 'coins' as IconName, title: 'Gagne 1 mois gratuit', desc: 'À chaque ami qui s\'abonne à Creator ou Pro.' },
      ],
      linkLabel: 'Ton lien de parrainage',
      copyLink: 'Copier le lien',
      copyLinkIcon: 'copy' as IconName,
      copied: 'Copié !',
      copiedIcon: 'check' as IconName,
      stats: {
        referred: 'Amis invités',
        converted: 'Abonnés payants',
        earned: 'Mois gratuits gagnés',
      },
      note: 'Seuls les abonnements Creator ou Pro comptent (pas les comptes Free).',
      noteIcon: 'alert-triangle' as IconName,
    },
    footer: {
      tagline: 'La création de contenu propulsée par l\'IA',
      rights: '© 2026 ViraReel AI. Tous droits réservés.',
    },
  },
  en: {
    nav: {
      logo: 'ViraReel AI',
      pricing: 'Pricing',
      referral: 'Referral',
      dashboard: 'Dashboard',
    },
    hero: {
      badge: 'A productivity tool for agencies and active creators',
      title: 'Complete scripts',
      titleGradient: 'ready to publish',
      titleEnd: 'in seconds',
      subtitle: 'Enter your idea and your post context (setting, visuals, mood): AI generates ultra-personalized hooks, scripts, captions and hashtags for all 4 platforms.',
      cta: 'Open the script generator',
      ctaSub: '12 free trials — No email, no credit card',
    },
    generator: {
      title: 'Script Generator',
      subtitle: 'Enter your idea along with your visual context, and the AI generates your content.',
      topicLabel: 'Topic, visuals & audience of your post',
      topicPlaceholder: 'E.g.: 3 marketing mistakes that cost you money. I speak to small business owners. Consultant for 8 years. Talking to camera in a modern office, tight shots on the product.',
      topicHelp: {
        title: 'What to write here',
        intro: 'Your topic and the context the AI cannot guess:',
        bullets: [
          { k: 'The topic', v: 'what you are about to publish' },
          { k: 'Who for', v: 'the audience you are speaking to' },
          { k: 'Who is speaking', v: 'job, experience, what makes them credible' },
          { k: 'The visuals', v: 'the setting and the style' },
        ],
        exCreator: 'E.g. creator: "3 marketing mistakes that cost you money. I speak to small business owners. Consultant for 8 years. Talking to camera in a modern office, tight shots on the product."',
        exAgency: 'E.g. agency — describe the client, not your agency: "Content for a family dental clinic. The owner-dentist presents it herself. We speak to local parents. Reassuring tone, filmed on site."',
        guard: 'Describe what you do, not what it guarantees.',
        note: 'In 4 ideas mode, this field keeps the context only: it applies to all 4.',
      },
      ideaHelp: {
        title: 'What to write here',
        intro: 'One specific topic per tab — no context, it is already above.',
        bullets: [
          '4 different topics, not 4 ways of saying the same thing',
          'A short sentence is enough',
          'Out of ideas? The "Suggest 4 angles" button fills them in for you, free and without using a trial. You can edit everything afterwards.',
        ],
        exContext: 'E.g. — in the field above: "I speak to small business owners. Consultant for 8 years. Talking to camera in a modern office, tight shots on the product."',
        exIdeasIntro: 'Then, in the tabs:',
        exIdeas: [
          'Idea 1 — The 3 Facebook ads mistakes',
          'Idea 2 — Why your website does not convert',
          'Idea 3 — The minimum budget to get started',
          'Idea 4 — How to write an irresistible offer',
        ],
      },
      toneHelp: {
        title: 'Which tone to choose',
        intro: 'The tone sets the voice of the script, not the subject:',
        bullets: [
          { k: 'Educational', v: 'problem → solution → key details. The best pick to sell a product or a service.' },
          { k: 'Expert', v: 'counter-intuitive claim → proof → method. For a professional audience, backed by a fact or a mechanism.' },
          { k: 'Inspirational', v: 'struggle → turning point → result. Requires telling your own story.' },
          { k: 'Witty', v: 'absurd premise, back-to-back punchlines. Made to be sent to a friend.' },
          { k: 'Trendy', v: 'a current format, present-day energy. Ages fast.' },
          { k: 'Emotional', v: 'tension → resolution. Also requires lived experience.' },
        ],
        note: 'When in doubt, pick Educational.',
      },
      helpOpen: 'What should I write in this field?',
      helpToneOpen: 'Which tone should I choose?',
      helpClose: 'Got it',
      platformLabel: 'Target platforms',
      platformHint: 'Select one or more',
      toneLabel: 'Tone/Style',
      regionLabel: 'Your target audience',
      regionLabelIcon: 'globe' as IconName,
      regions: {
        'us': 'US · United States',
        'uk': 'UK · United Kingdom',
        'au': 'AU · Australia',
        'ca-en': 'CA · Canada (English)',
        'other-en': 'International English',
      } as Record<string, string>,
      tones: {
        educational: 'Educational',
        expert: 'Expert',
        inspirational: 'Inspirational',
        funny: 'Witty',
        trendy: 'Trendy',
        emotional: 'Emotional',
      },
      tonesIcons: {
        educational: 'book-open',
        expert: 'briefcase',
        inspirational: 'flame',
        funny: 'laugh',
        trendy: 'sparkles',
        emotional: 'heart',
      } as Record<string, IconName>,
      platforms: {
        instagram: 'Instagram Reels',
        tiktok: 'TikTok',
        facebook: 'Facebook Reels',
        youtube: 'YouTube Shorts',
        all: '4 Platforms',
      },
      // Official brand logos ; `all` = the 4 platforms (grid), not the language switch.
      platformsIcons: {
        instagram: 'instagram',
        tiktok: 'tiktok',
        facebook: 'facebook',
        youtube: 'youtube',
        all: 'layout-grid',
      } as Record<string, IconName>,
      generateBtn: 'Generate my script',
      generateBtnIcon: 'zap' as IconName,
      variationsBtn: 'Generate 3 script variations',
      variationsBtnIcon: 'refresh-cw' as IconName,
      ideasBtn: 'Generate 4 ideas',
      ideasBtnIcon: 'lightbulb' as IconName,
      generating: 'Generating...',
      remaining: 'trials remaining',
      upgradeBtn: 'Upgrade to Creator',
      upgradeBtnIcon: 'rocket' as IconName,
      results: {
        hook: 'Punchy Hook',
        hookIcon: 'magnet' as IconName,
        hookSub: 'Your first 3 seconds = everything',
        script: 'Step-by-Step Script',
        scriptIcon: 'file-text' as IconName,
        scriptSub: 'Max 45 seconds read',
        screenText: 'On-Screen Text',
        screenTextIcon: 'pencil' as IconName,
        screenTextSub: 'One punchy line per page',
        visualInspo: 'Visual Inspiration',
        visualInspoIcon: 'lightbulb' as IconName,
        visualInspoSub: 'Shot ideas to film it (optional)',
        caption: 'Caption + Hashtags',
        captionIcon: 'message-circle' as IconName,
        captionSub: 'Ready to copy-paste',
        duration: 'Suggested Duration',
        durationIcon: 'timer' as IconName,
        durationSub: 'Optimized for TikTok',
        // Here 🎵 meant SOUND, not TikTok -> Music (not the TikTok logo).
        trend: 'Sound Trend',
        trendIcon: 'music' as IconName,
        trendSub: 'To maximize your reach',
        bestTime: 'Best Time to Post',
        bestTimeIcon: 'clock' as IconName,
        bestTimeSub: 'To maximize views',
        ytTitle: 'Optimized Title',
        ytTitleIcon: 'tag' as IconName,
        ytTitleSub: 'For YouTube SEO ranking',
        seoDescription: 'SEO Description',
        seoDescriptionIcon: 'search' as IconName,
        seoDescriptionSub: 'To maximize discoverability',
        keywords: 'Keywords / Tags',
        keywordsIcon: 'key' as IconName,
        keywordsSub: 'YouTube tags for more views',
        variation: 'Variation',
        copyBtn: 'Copy',
        copyBtnIcon: 'copy' as IconName,
        copied: 'Copied!',
        copiedIcon: 'check' as IconName,
      },
    },
    pricing: {
      title: 'Our plans',
      subtitle: 'Monthly or annual billing',
      monthly: 'Monthly',
      annual: 'Annual',
      save: '2 months free',
      perMonth: '/month',
      perYear: '/year',
      plans: {
        solo: {
          name: 'Solo',
          desc: 'To get started',
          features: [
            '60 generations/month',
            'Hook + Script + Screen text + Caption + Hashtags',
            'Visual inspiration per platform',
            'TikTok sound trends',
            'Best posting time',
            'Option of 3 variations per generation',
            '6 tones',
            '4 platforms',
            'Your last 20 generations (saved on your device)',
            'Export — choice of 3 formats (TXT / MD / CSV)',
            'Bilingual transcreation in 1 click (FR ↔ EN, region-adapted)',
          ],
          cta: 'Start Solo',
          badge: '',
        },
        creator: {
          name: 'Creator',
          desc: 'For ambitious creators',
          features: [
            '160 generations/month',
            'Hook + Script + Screen text + Caption + Hashtags',
            'Visual inspiration per platform',
            'TikTok sound trends',
            'Best posting time',
            'Option of 3 variations per generation',
            '6 tones',
            '4 platforms',
            'Your last 40 generations, fully viewable (saved on your device)',
            'Export — choice of 3 formats (TXT / MD / CSV)',
            'Bilingual transcreation in 1 click (FR ↔ EN, region-adapted)',
            '4 ideas × 4 platforms in 1 click (16 pieces of content at once)',
          ],
          cta: 'Start Creator',
          badge: 'Popular',
          badgeIcon: 'star' as IconName,
        },
        pro: {
          name: 'Agency',
          desc: 'For agencies and teams',
          features: [
            '1000 generations/month',
            'Hook + Script + Screen text + Caption + Hashtags',
            'Visual inspiration per platform',
            'TikTok sound trends',
            'Best posting time',
            'Option of 3 variations per generation',
            '6 tones',
            '4 platforms',
            'Your last 150 generations, fully viewable (saved on your device)',
            'Export — choice of 3 formats (TXT / MD / CSV)',
            'Bilingual transcreation in 1 click (FR ↔ EN, region-adapted)',
            '4 ideas × 4 platforms in 1 click (16 pieces of content at once)',
            'Shared account for your team',
            'Priority support',
          ],
          cta: 'Start Agency',
        },
      },
      founder: {
        banner: 'LAUNCH OFFER · Founding member pricing',
        bannerSub: 'The first 50 subscribers lock in their price FOR LIFE.',
        spots: 'founding member spots left',
        spotsOnly: '50 spots only',
        barSpots: '50 spots',
        lastSpots: 'Only {n} founding spots left!',
        lastSpotOne: 'Only 1 founding spot left!',
        lastSpotsIcon: 'flame' as IconName,
        closedIcon: 'star' as IconName,
        closedTitle: 'All 50 founding spots are taken.',
        closedThanks: 'Thank you to the first 50 creators who joined ViraReel AI from day one.',
        closedOver: 'The founding offer is now closed.',
        closedRegular: 'Regular pricing now applies.',
        tag: 'Founding member',
        notice: 'For life on the plan you subscribed to — not transferable if you switch plans.',
        lifetime: 'for life',
        was: 'Regular price',
        cta: 'Lock in my price for life',
        launch: 'Launch offer',
        bar: 'Price locked for life',
        seeBelow: 'See below',
      },
    },
    referral: {
      title: 'Referral Program',
      titleIcon: 'gift' as IconName,
      subtitle: 'Share ViraReel and earn free months!',
      howTitle: 'How does it work?',
      steps: [
        { icon: 'link' as IconName, title: 'Copy your unique link', desc: 'Every account has a personal referral link.' },
        { icon: 'share' as IconName, title: 'Share it', desc: 'Send your link to friends, followers, or on social media.' },
        { icon: 'coins' as IconName, title: 'Earn 1 free month', desc: 'For every friend who subscribes to Creator or Pro.' },
      ],
      linkLabel: 'Your referral link',
      copyLink: 'Copy link',
      copyLinkIcon: 'copy' as IconName,
      copied: 'Copied!',
      copiedIcon: 'check' as IconName,
      stats: {
        referred: 'Friends invited',
        converted: 'Paid subscribers',
        earned: 'Free months earned',
      },
      note: 'Only Creator or Pro subscriptions count (not Free accounts).',
      noteIcon: 'alert-triangle' as IconName,
    },
    footer: {
      tagline: 'AI-powered content creation',
      rights: '© 2026 ViraReel AI. All rights reserved.',
    },
  },
};

export type Translations = typeof translations.fr;
